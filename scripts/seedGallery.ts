...
const MONGODB_URI = "mongodb+srv://PRASANNA:AMET%401234567890@mnrs.hg6yavf.mongodb.net/?appName=MNRS";

type GalleryCategory = "Campus" | "Classrooms" | "Sports" | "Events" | "Residential";
...
const GallerySchema = new mongoose.Schema<IGalleryItem>(
  {
...
const GalleryItem = mongoose.models.GalleryItem || mongoose.model<IGalleryItem>("GalleryItem", GallerySchema);
...
async function seedGallery() {
  try {
    ...
    // Insert new items
    const inserted = await GalleryItem.insertMany(galleryItems);
    console.log(`✓ Successfully inserted ${inserted.length} gallery items`);

    // Log items by category
    const categories = ["Campus", "Classrooms", "Sports", "Events", "Residential"];
    for (const category of categories) {
      const count = await GalleryItem.countDocuments({ category });
